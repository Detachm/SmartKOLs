...
export interface QueuePullDirectMessagesJobDependencies {
...
export class QueuePullDirectMessagesJob {
  constructor(private readonly deps: QueuePullDirectMessagesJobDependencies) {}
  ...
  async execute(accountId: string) {
    ...
    const existingQueued = await this.deps.workerJobs.findQueuedByTypeAndTarget("dm.pull", "account", account.id);
    if (existingQueued) {
