...
export interface RepostPostDependencies {
...
export class RepostPost {
  constructor(private readonly deps: RepostPostDependencies) {}

  async execute(input: { account_id: string; target_post_id: string; idempotency_key_override?: string }) {
    ...
    const targetPostId = input.target_post_id.trim();
    const now = this.deps.clock.now().toISOString();
    const requestId = newId();
    const idempotencyKey = input.idempotency_key_override?.trim()
      || createConnectorIdempotencyKey([account.id, "post.repost", targetPostId]);
    const runningLedger = await reserveConnectorRequest({
      ...
      request: createConnectorRequest({
        ...
        request_payload: JSON.stringify({ target_post_id: targetPostId }),
        status: "running",
        started_at: now,
      }),
    });
    const result = await this.repostPostOnTwitter({
      account_id: account.id,
    ...
    try {
      ...
      throw new AppError("INTERNAL_ERROR", "repost succeeded but local persistence failed", {
        cause: error,
        details: {
          account_id: account.id,
  ...
  private async repostPostOnTwitter(
    input: {
      account_id: string;
    ...
    try {
      return await this.deps.twitterClient.repostPost(input);
    } catch (error) {
      const appError = error instanceof AppError
        ? error
        : new AppError("EXTERNAL_DEPENDENCY_ERROR", "twitter repostPost failed", { cause: error });
      ...
      try {
        ...
        throw new AppError("INTERNAL_ERROR", "twitter repostPost failed and failure could not be recorded", {
