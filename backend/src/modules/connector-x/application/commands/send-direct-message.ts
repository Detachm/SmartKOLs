...
export class SendDirectMessage {
  constructor(private readonly deps: SendDirectMessageDependencies) {}
  ...
  async execute(input: {
    ...
    const result = await this.sendDirectMessageOnTwitter({
      account_id: account.id,
    ...
    try {
      ...
      throw new AppError("INTERNAL_ERROR", "direct message was sent but local persistence failed", {
        cause: error,
        details: {
          account_id: account.id,
  ...
  private async sendDirectMessageOnTwitter(
    input: {
      account_id: string;
    ...
    try {
      return await this.deps.twitterClient.sendDirectMessage(input);
    } catch (error) {
      const appError = error instanceof AppError
        ? error
        : new AppError("EXTERNAL_DEPENDENCY_ERROR", "twitter sendDirectMessage failed", { cause: error });
      ...
      try {
        ...
        throw new AppError("INTERNAL_ERROR", "twitter sendDirectMessage failed and failure could not be recorded", {
