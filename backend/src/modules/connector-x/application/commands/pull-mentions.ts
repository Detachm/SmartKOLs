...
export interface PullMentionsDependencies {
...
export class PullMentions {
  constructor(private readonly deps: PullMentionsDependencies) {}
  ...
  async execute(accountId: string) {
    ...
    const result = await this.pullMentionsFromTwitter(accountId, credential.provider, credential.secret_ref, {
    ...
    try {
      ...
      throw new AppError("INTERNAL_ERROR", "mentions were pulled but local persistence failed", {
        cause: error,
        details: {
          account_id: accountId,
          request_id: requestId,
  ...
  private async pullMentionsFromTwitter(
    accountId: string,
    ...
    try {
      return await this.deps.twitterClient.listMentions({
        account_id: accountId,
        provider,
      ...
      const appError = error instanceof AppError
        ? error
        : new AppError("EXTERNAL_DEPENDENCY_ERROR", "twitter listMentions failed", { cause: error });
      ...
      try {
        ...
        throw new AppError("INTERNAL_ERROR", "twitter mentions pull failed and failure could not be recorded", {
