...
export interface CommentOnPostDependencies {
...
export class CommentOnPost {
  constructor(private readonly deps: CommentOnPostDependencies) {}
  ...
  async execute(input: {
    ...
    const targetPostId = input.comment_on_external_post_id.trim();
    const text = input.text.trim();
    const now = this.deps.clock.now().toISOString();
    const requestId = newId();
    const idempotencyKey = input.idempotency_key_override?.trim()
      || createConnectorIdempotencyKey([account.id, "post.comment", targetPostId, text]);
    const runningLedger = await reserveConnectorRequest({
      ...
      request: createConnectorRequest({
        ...
        request_payload: JSON.stringify({
          comment_on_external_post_id: targetPostId,
          text,
        }),
        status: "running",
        started_at: now,
    ...
    const result = await this.commentOnPostOnTwitter({
      account_id: account.id,
    ...
    try {
      ...
      throw new AppError("INTERNAL_ERROR", "comment was created but local persistence failed", {
        cause: error,
        details: {
          account_id: account.id,
  ...
  private async commentOnPostOnTwitter(
    input: {
      account_id: string;
    ...
    try {
      return await this.deps.twitterClient.commentOnPost(input);
    } catch (error) {
      const appError = error instanceof AppError
        ? error
        : new AppError("EXTERNAL_DEPENDENCY_ERROR", "twitter commentOnPost failed", { cause: error });
      ...
      try {
        ...
        throw new AppError("INTERNAL_ERROR", "twitter commentOnPost failed and failure could not be recorded", {
