import { AppError } from "../../../../core/errors/app-error";
import { newId } from "../../../../core/ids/new-id";
import type { Clock } from "../../../../core/time/clock";
import type { AuditLogRepository } from "../../../audit/application/ports/audit-log-repository";
import type { SourcesRepository } from "../ports/sources-repository";
import { pauseSource } from "../../domain/source";

export interface PauseSourceDependencies {
  sources: SourcesRepository;
  auditLogs: AuditLogRepository;
  clock: Clock;
}

export class PauseSource {
  constructor(private readonly deps: PauseSourceDependencies) {}

  async execute(sourceId: string) {
    const source = await this.deps.sources.findSourceById(sourceId);
    if (!source) {
      throw new AppError("NOT_FOUND", "source not found", {
        details: { source_id: sourceId },
      });
    }

    const paused = pauseSource(source);
    await this.deps.sources.saveSource(paused);
    await this.deps.auditLogs.append({
      id: newId(),
      workspace_id: paused.workspace_id,
      actor_type: "user",
      entity_type: "source",
      entity_id: paused.id,
      action: "source.paused",
      before_state: JSON.stringify(source),
      after_state: JSON.stringify(paused),
      created_at: this.deps.clock.now().toISOString(),
    });

    return paused;
  }
}
